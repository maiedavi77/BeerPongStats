...
// Rows must match the rack_position → coordinate mapping in cup-rack.js
// (GRID10 / GRID6): the wide row is positions 0..3 / 0..2, narrowing to the
// single tip cup (9 / 5).
const LAYOUTS = {
  10: [
    [0, 1, 2, 3],
...
function buildHitMap(throws, cups) {
  ...
  for (const t of throws) {
    // A dodge is a hit too. Line-lane cups (rack_position >= 100) are transient
    // re-rack artifacts and are skipped below.
    if (t.outcome !== 'hit' && t.outcome !== 'dodge') continue;

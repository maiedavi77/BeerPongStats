...
async function loadGames() {
  ...
  // Build cup-count map: { gameId: { A: n, B: n } }
  const cupMap = {};
  for (const c of cups ?? []) {
    if (!cupMap[c.game_id]) cupMap[c.game_id] = { A: 0, B: 0 };
    cupMap[c.game_id][c.team] = (cupMap[c.game_id][c.team] || 0) + 1;
  }

  $list.innerHTML = games.map(game => renderGameCard(game, cupMap[game.id] ?? { A: 0, B: 0 })).join('');
    ...
    " onmouseenter="this.style.background='var(--surface-2)'"
      ...
      <div style="display:flex; align-items:center; gap:1rem;">
        <div style="flex:1; text-align:center;">
          <div style="font-family:'Bebas Neue',sans-serif; font-size:2.5rem; color:var(--blue); line-height:1;">${cups.A}</div>
        ...
        <div style="flex:1; text-align:center;">
          <div style="font-family:'Bebas Neue',sans-serif; font-size:2.5rem; color:var(--amber); line-height:1;">${cups.B}</div>
